...
export default function MarkdownRenderer({
  ...
  const [signedUrls, setSignedUrls] = useState<Map<string, ImageUrls>>(
    new Map()
  );
  ...
  useEffect(() => {
    async function loadSignedUrls() {
      ...
      try {
        ...
        const pathToImageId: Map<string, { imageId: string; variant: string }> =
          new Map();
        ...
        images.forEach((img) => {
          if (img.storagePaths) {
            ...
            pathToImageId.set(img.storagePaths.original, {
              imageId: img.id,
              variant: "original",
            });
            pathToImageId.set(img.storagePaths.medium, {
              imageId: img.id,
              variant: "medium",
            });
            pathToImageId.set(img.storagePaths.thumb, {
              imageId: img.id,
              variant: "thumb",
            });
          }
    ...
    imageMatches.forEach((imgMatch, idx) => {
      ...
      if (image && urls && urls.medium) {
        ...
        parts.push(
          <div
            key={`img-loading-${idx}`}
            className="my-4 p-8 bg-gray-100 dark:bg-gray-800 rounded-lg text-center"
          >
        ...
        parts.push(
          <div
            key={`img-missing-${idx}`}
            className="my-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg text-center text-gray-500"
          >
...
function parseMarkdown(text: string): string {
  return (
    text
      ...
      .replace(
        /```([\s\S]*?)```/g,
        '<pre class="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg overflow-x-auto my-2"><code>$1</code></pre>'
      )
      .replace(
        /`(.*?)`/g,
